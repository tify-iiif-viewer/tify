...
export function checkTranslationFiles(
	...
	translationFiles.forEach((file) => {
		...
		keys.forEach((key) => {
			if (!translation[key]) {
				result.issues.empty.push(key);
			}
		});

		result.issues.missing = referenceKeys.filter((key) => !keys.includes(key));
		result.issues.unused = keys.filter((key) => !referenceKeys.includes(key));

		const keysStr = keys.join(', ');
		const referenceKeysStr = referenceKeys.join(', ');
		if (!result.issues.missing.length
			&& !result.issues.unused.length
		...
		if (options.addMissing && result.issues.missing.length) {
			result.issues.missing.forEach((key) => {
				translation[key] = '';
			});
			result.notes.push('Added missing keys, please add translations');
		}

		if (options.removeUnused && result.issues.unused.length) {
			result.issues.unused.forEach((key) => {
		...
		if (options.addMissing || options.removeUnused || options.sort) {
			const updatedTranslation = Object.keys(translation)
				.sort((a, b) => {
					if (a === '$language') {
						return -1;
					}
