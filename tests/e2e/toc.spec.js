describe('TOC', () => {
	it('allows navigation', () => {
		const manifestUrl = `${Cypress.expose('iiifApiUrl')}/manifests/gdz-HANS_DE_7_w042081.json`;
	...
	it('is working even when structures are chaotic', () => {
		const manifestUrl = `${Cypress.expose('iiifApiUrl')}/manifests/cambridge-MS-ADD-08640.json`;
	...
	it('hides items if their "behavior" is "top"', () => {
		const manifestUrl = `${Cypress.expose('iiifApiUrl')}/manifests/digitale-sammlungen-bsb00026283.json`;
		const encodedParams = encodeURIComponent(JSON.stringify({
			view: 'toc',
		}));
	...
	it('hides the whole TOC if there are no structures', () => {
		const manifestUrl = `${Cypress.expose('iiifApiUrl')}/manifests/aku-pal-375.json`;
	...
	it('displays page labels instead of structure labels if the latter are missing', () => {
		const manifestUrl = `${Cypress.expose('iiifApiUrl')}/manifests/bodleian-faeff7fb-f8a7-44b5-95ed-cff9a9ffd198.json`;
		const encodedParams = encodeURIComponent(JSON.stringify({
			view: 'toc',
		}));
