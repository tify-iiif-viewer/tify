describe('Support for native IIIF 3 manifests', () => {
	it('displays correct metadata', () => {
		cy.visit(
			`/?manifest=${Cypress.expose('iiifApiUrl')}/manifests/aku-pal-375.json`
			+ '&tify={"view":"info"}',
		);
	...
	it('displays the TOC', () => {
		cy.visit(
			`/?manifest=${Cypress.expose('iiifApiUrl')}/manifests/amherst-63cc6105-570d-407b-af8c-07fda3f8c620.json`
			+ '&tify={"view":"toc"}',
		);
	...
	it('displays attribution HTML', () => {
		cy.visit(
			`/?manifest=${Cypress.expose('iiifApiUrl')}/manifests/mskgent-8210.json`
			+ '&tify={"view":"info"}',
		);
	...
	it('displays a dash for metadata with an empty value', () => {
		cy.visit(
			`/?manifest=${Cypress.expose('iiifApiUrl')}/manifests/utrecht-1874-325480.json`
			+ '&tify={"view":"info"}',
		);
