	...
	methods: {
		...
		updateOverlays() {
			...
			this.$store.options.pages.forEach((page, pageIndex) => {
				...
				this.$store.annotations[page]?.forEach((annotation) => {
					// TODO: Add aria-label or something like that
					const element = document.createElement('a');

					element.className = 'tify-scan-overlay';
					element.className +=	this.$store.options.annotationId === annotation.id
						? ' -current'
						: '';
